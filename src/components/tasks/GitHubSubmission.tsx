import { useState } from 'react';
import { motion } from 'framer-motion';
import { Globe, CheckCircle, AlertCircle } from 'lucide-react';
import type { TaskSubmission } from '../../context/CareerContext';

interface GitHubSubmissionProps {
  nodeId: string;
  submission: TaskSubmission;
  onSubmit: (data: { githubUrl: string; liveUrl?: string }) => void;
  githubRequired: boolean;
  liveUrlRequired: boolean;
}

function isValidGitHubUrl(url: string): boolean {
  return /^https?:\/\/(www\.)?github\.com\/[^/]+\/[^/]+/.test(url);
}

function isValidUrl(url: string): boolean {
  try {
    new URL(url);
    return true;
  } catch {
    return false;
  }
}

export function GitHubSubmission({
  submission,
  onSubmit,
  githubRequired,
  liveUrlRequired,
}: GitHubSubmissionProps) {
  const [githubUrl, setGithubUrl] = useState(submission.githubUrl || '');
  const [liveUrl, setLiveUrl] = useState(submission.liveUrl || '');
  const [touched, setTouched] = useState({ github: false, live: false });

  const isSubmitted = submission.status === 'submitted' || submission.status === 'under-review' || submission.status === 'passed' || submission.status === 'failed';

  const githubValid = !githubRequired || (githubUrl.length > 0 && isValidGitHubUrl(githubUrl));
  const liveValid = !liveUrlRequired || (liveUrl.length > 0 && isValidUrl(liveUrl));
  const canSubmit = githubValid && liveValid && !isSubmitted;

  const statusConfig = {
    'not-started': { label: 'Not Submitted', color: 'text-muted-foreground', bg: 'bg-muted', emoji: '○' },
    'in-progress': { label: 'In Progress', color: 'text-warning', bg: 'bg-warning/10', emoji: '◐' },
    'submitted': { label: 'Submitted', color: 'text-info', bg: 'bg-info/10', emoji: '✓' },
    'under-review': { label: 'Under Review', color: 'text-primary', bg: 'bg-primary/10', emoji: '👁' },
    'passed': { label: 'Passed', color: 'text-success', bg: 'bg-success/10', emoji: '✓' },
    'failed': { label: 'Failed', color: 'text-danger', bg: 'bg-danger/10', emoji: '✗' },
  };

  const status = statusConfig[submission.status];

  return (
    <div className="space-y-6">
      {/* Submission Status */}
      <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold ${status.bg} ${status.color}`}>
        <span>{status.emoji}</span>
        {status.label}
      </div>

      {isSubmitted ? (
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          className="p-6 rounded-2xl border border-success/30 bg-success/5 space-y-4"
        >
          <div className="flex items-center gap-3 mb-4">
            <CheckCircle size={24} className="text-success" />
            <h3 className="font-display text-xl font-medium text-foreground">Submission successful.</h3>
          </div>
          {submission.githubUrl && (
            <div className="flex items-center justify-between py-3 border-b border-border">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <Globe size={15} /> GitHub Repository
              </div>
              <div className="flex items-center gap-1.5 text-success text-xs font-medium">
                <CheckCircle size={13} /> Submitted
              </div>
            </div>
          )}
          {submission.liveUrl && (
            <div className="flex items-center justify-between py-3 border-b border-border">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <Globe size={15} /> Live Project
              </div>
              <div className="flex items-center gap-1.5 text-success text-xs font-medium">
                <CheckCircle size={13} /> Submitted
              </div>
            </div>
          )}
          <div className="flex items-center justify-between py-3">
            <span className="text-sm text-muted-foreground">Status</span>
            <span className={`text-sm font-semibold ${status.color}`}>{status.label}</span>
          </div>
        </motion.div>
      ) : (
        <div className="space-y-5">
          {/* GitHub URL */}
          <div>
            <label className="block text-xs uppercase tracking-widest text-muted-foreground font-semibold mb-2">
              GitHub Repository {githubRequired && <span className="text-danger">*</span>}
            </label>
            <div className="relative">
              <Globe size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground" />
              <input
                type="url"
                value={githubUrl}
                onChange={(e) => setGithubUrl(e.target.value)}
                onBlur={() => setTouched((t) => ({ ...t, github: true }))}
                placeholder="https://github.com/username/project"
                className="w-full pl-10 pr-4 py-3 bg-surface border border-border rounded-xl text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/50 transition-all"
              />
            </div>
            {touched.github && !githubUrl && githubRequired && (
              <p className="flex items-center gap-1.5 text-xs text-danger mt-1.5">
                <AlertCircle size={12} /> Please enter a GitHub repository URL.
              </p>
            )}
            {touched.github && githubUrl && !isValidGitHubUrl(githubUrl) && (
              <p className="flex items-center gap-1.5 text-xs text-danger mt-1.5">
                <AlertCircle size={12} /> Please enter a valid GitHub repository URL.
              </p>
            )}
            {touched.github && githubUrl && isValidGitHubUrl(githubUrl) && (
              <p className="flex items-center gap-1.5 text-xs text-success mt-1.5">
                <CheckCircle size={12} /> Valid GitHub repository URL
              </p>
            )}
          </div>

          {/* Live URL */}
          <div>
            <label className="block text-xs uppercase tracking-widest text-muted-foreground font-semibold mb-2">
              Live Project URL {liveUrlRequired && <span className="text-danger">*</span>}
              {!liveUrlRequired && <span className="text-muted-foreground font-normal normal-case tracking-normal ml-1">(optional)</span>}
            </label>
            <div className="relative">
              <Globe size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground" />
              <input
                type="url"
                value={liveUrl}
                onChange={(e) => setLiveUrl(e.target.value)}
                onBlur={() => setTouched((t) => ({ ...t, live: true }))}
                placeholder="https://my-project.vercel.app"
                className="w-full pl-10 pr-4 py-3 bg-surface border border-border rounded-xl text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/50 transition-all"
              />
            </div>
            {touched.live && liveUrl && !isValidUrl(liveUrl) && (
              <p className="flex items-center gap-1.5 text-xs text-danger mt-1.5">
                <AlertCircle size={12} /> Please enter a valid URL
              </p>
            )}
            {touched.live && liveUrl && isValidUrl(liveUrl) && (
              <p className="flex items-center gap-1.5 text-xs text-success mt-1.5">
                <CheckCircle size={12} /> Valid project URL
              </p>
            )}
          </div>

          {/* Submit button */}
          <motion.button
            onClick={() => { if (canSubmit) onSubmit({ githubUrl, liveUrl: liveUrl || undefined }); }}
            disabled={!canSubmit}
            whileHover={canSubmit ? { scale: 1.02 } : {}}
            whileTap={canSubmit ? { scale: 0.98 } : {}}
            className="w-full py-3.5 bg-primary text-primary-foreground rounded-xl font-semibold text-sm hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
          >
            Submit Project
          </motion.button>
          {/* TODO: Connect to backend API for actual submission */}
        </div>
      )}
    </div>
  );
}
